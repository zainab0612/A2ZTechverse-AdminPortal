...
export const ProjectTable = ({ variant, showMembers, actions, menu, status, description }: ProjectTableProps) => {
    const bgClass =
        variant === "dashboard"
            ? "bg-linear-to-br from-[#0d0f24] to-[#185672]"
            : "bg-linaer-to-br from-[#0A124A] to-[#0A0D2D]";
    return (
        <>
            <div className={`rounded-2xl p-6 shadow-2xl w-full ${bgClass}`}>
                <div className="flex justify-between mb-6">
                    <div>
                        <h1 className="font-bold text-xl">Projects</h1>
                        {description && (
                            <p className="text-white/60 text-sm flex gap-2"><Check /> 30 done this month</p>
                        )}
                    </div>
                    <DropdownMenu>
                        {menu && (
